# Doll Up Hair & Nail Art — Backend Guide (Supabase)
...
| | |
|---|---|
| Project name | `doll-up-hair-and-nail-art` |
...
2. On the dashboard, open the **`doll-up-hair-and-nail-art`** project.
...
Client books on the website
 ...
 You open /admin  ── Confirm ──►  │  (status: confirmed)
        │                         │
        └── Mark complete ──►  a Crown Card STAMP is created automatically
                                   │
                                   ▼
                             stamps table
```
...
3. To add a service: **Insert → Insert row**, then fill in:
   - `id` — a short lowercase label, no spaces (e.g. `knotless-medium`)
   - `category` — one of `hair`, `nails`, or `kids`
