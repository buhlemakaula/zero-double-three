...
// Public per-booking "welcome" data for the /welcome/<id> link. The address is
// only present once the booking is confirmed (enforced server-side).
export async function fetchBookingPublic(id) {
  if (isSupabaseConfigured) {
    const { data, error } = await supabase.rpc('booking_public', { b_id: id })
    if (!error && data) return data
    if (error) throw error
  }
  const b = memoryBookings.find((x) => x.id === id)
  return b ? { found: true, ...b } : { found: false }
}

// Client submits a review (only valid for a completed booking).
...
// Approved 4–5 star reviews for the site testimonials.
...
// The Glow Line: a client asks Doll Up to call them.
export async function submitEnquiry({ name, phone, message, preferred }) {
  if (isSupabaseConfigured) {
