...
import { SERVICE_TABS, DIVIDER_CATEGORY, servicesByCategory } from '../data/services.js'
...
export default function Services({ settings, onBook }) {
  ...
  const callouts = servicesByCategory(DIVIDER_CATEGORY.id)
      ...
      <div className="shell">
        ...
        <div className="mt-20">
          <div className="mb-10 flex items-center gap-5">
            ...
            <h3 className="font-display text-3xl uppercase tracking-tight sm:text-4xl">Kids.</h3>
            <span className="h-px flex-1 bg-ink/15" />
          </div>
          <div className="mx-auto grid max-w-2xl grid-cols-2 gap-x-6 gap-y-12 sm:grid-cols-3">
            {callouts.map((s) => (
              <ServiceCard
                key={s.id}
                service={s}
