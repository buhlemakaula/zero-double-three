...
(function () {
  ...
  var PRODUCTS = [
    ...
    { id:"bubble-tee", cat:"Tee", name:"Bubble Logo Tee", price:399, was:499, img:"img/blue-logo-tee.jpg", flag:"Sale", swatch:["#0E0E10","#F4F1EA"] },
    ...
    { id:"duo-set", cat:"Two-piece", name:"Court Lounge Set", price:999, was:1149, img:"img/duo-grey-steps.jpg", flag:"Sale", swatch:["#9A979A"] }
  ];
  ...
  if (grid) {
    grid.innerHTML = PRODUCTS.map(function (p, i) {
      ...
      var delay = (i % 4) + 1;
      var isSale = p.flag && /sale/i.test(p.flag);
      var priceHtml = p.was
        ? '<span class="card__price"><s style="color:var(--grey-2);font-weight:400;margin-right:.4em">' + rand(p.was) + '</s><span style="color:var(--red)">' + rand(p.price) + '</span></span>'
        : '<span class="card__price">' + rand(p.price) + '</span>';
      return '' +
      '<article class="card" data-reveal data-reveal-delay="' + delay + '">' +
        '<div class="card__media">' +
          (p.flag ? '<span class="card__flag' + (isSale ? ' is-sale' : '') + '">' + p.flag + '</span>' : '') +
          '<button class="card__wish" aria-label="Save ' + p.name + '"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M12 21C5 16 3 12 3 8.5A4.5 4.5 0 0 1 12 6a4.5 4.5 0 0 1 9 2.5C21 12 19 16 12 21z"/></svg></button>' +
